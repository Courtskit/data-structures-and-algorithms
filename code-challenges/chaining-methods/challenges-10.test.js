...
const createServer = () => {
  ...
  app.get('/hello', (request, response) => {
    response.status(200).send('Hello! Hey')
  })

  app.get('/aboutme', (request, response) => {
    response.status(200).send('Hey! My name is Courtney. I live in the Seattle area and I like to code.')
  })

  app.get('/favoritefoods', (request, response) => {
    response.status(200).send(['veggies', 'tacos', 'poke']);
  })
...
const count = (target, input) => {
  let count = 0;
  count = input.reduce((counter, index) => {
    let number = index.reduce((counter2, index2) => {
      if (index2 === target) counter2++;
      return counter2;
    }, 0);
    counter += number;
    return counter;
